package com.macstab.sky.user.web.rest;
...
import com.macstab.sky.user.IntegrationTest;
import com.macstab.sky.user.config.EmbeddedKafka;
import com.macstab.sky.user.config.KafkaSseConsumer;
import com.macstab.sky.user.config.KafkaSseProducer;
...
class UserserviceKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/userservice-kafka/publish?message=value-produce")).andExpect(status().isOk());
